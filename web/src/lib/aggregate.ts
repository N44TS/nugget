import type { AgeBand, Contribution, ContributionBatch } from "./types"
...
export type AggregateReport = {
  ...
  /** Share of pool in each age band — only bands with count >= kMin */
  ageBandShare: Record<string, number> | null
  /** Average cycle length per age band — only bands with count >= kMin */
  ...
  /** Signal category shares calculated from real contributor submissions. */
  wellbeingDistributions: Record<string, Record<string, number>> | null
  /** Observed joint shares, not relative-risk estimates. */
...
const isValidWellbeing = (signals: NonNullable<Contribution["wellbeing"]>): boolean =>
...
export const aggregateContributions = (batch: ContributionBatch, kMin: number): AggregateReport => {
  ...
  for (const c of valid) {
    const band = c.ageBand as AgeBand
    bandCounts[band] = (bandCounts[band] ?? 0) + 1
    bandCycleSum[band] = (bandCycleSum[band] ?? 0) + c.cycleLengthDays
  }
  ...
  for (const [bucket, matches] of Object.entries(cycleBuckets)) {
  ...
  return {
    ...
    ageBandStats: Object.keys(ageBandStats).length ? ageBandStats : null,
    cycleLengthStats: Object.keys(cycleLengthStats).length ? cycleLengthStats : null,
...
export const formatPublicSummary = (report: AggregateReport): string => {
  ...
  const coOccurrence = report.coOccurrenceRates
    ? Object.entries(report.coOccurrenceRates)
        ...
        .join(",")
    : ""
  return `OK batch=${report.epoch} n=${report.contributorCount} avgCycle=${report.avgCycleLength} avgPeriod=${report.avgPeriodLength} symptoms={${symptoms}} ageShare={${ages}} avgCycleByAge={${avgByAge}} rejected=${report.rejectedCount} wellbeing={${wellbeing}} coOccurrence={${coOccurrence}} ageStats={${encodeSegments(report.ageBandStats)}} cycleStats={${encodeSegments(report.cycleLengthStats)}} painRate=${report.moderateSeverePainRate ?? "x"}`
}
