...
function abytes2(value, length, title = "") {
  if (isBytes3(value) && (length === undefined || value.length === length))
    return value;
  if (length !== undefined)
...
function bytesToNumberBE2(bytes) {
  return hexToNumber4(bytesToHex3(bytes));
}
function bytesToNumberLE2(bytes) {
  return hexToNumber4(bytesToHex3(copyBytes(abytes2(bytes)).reverse()));
}
...
function bitLen2(n) {
  ...
  return n === _0n6 ? 0 : n.toString(2).length;
}
var bitMask2 = (n) => {
  asafenumber(n, "n");
...
function validateMSMScalars2(scalars, field, maxScalar) {
  if (!Array.isArray(scalars))
    throw new Error("array of scalars expected");
  scalars.forEach((s, i) => {
    const ok = maxScalar === undefined ? field.isValid(s) : isPosBig2(s) && s < maxScalar;
    if (!ok)
      throw new Error("invalid scalar at index " + i);
  });
}
...
function mulAddUnsafe(c, points, scalars, allowOversized = false) {
  ...
  validateMSMScalars2(scalars, c.Fn, allowOversized ? c.Fn.ORDER ** _4n4 : undefined);
...
function edwards(params, extraOpts = {}) {
  ...
  const randomBytes = opts.randomBytes === undefined ? randomBytes3 : opts.randomBytes;
  const MASK = _2n6 << BigInt(Fp.BYTES * 8) - _1n9;
  function isOdd(n) {
    if (!Fp.isOdd)
  ...
  class Point {
    ...
    static fromHex(hex, zip215 = false) {
      return Point.fromBytes(hexToBytes5(hex), zip215);
    }
    get x() {
      return this.toAffine().x;
    }
    ...
    toAffine(invertedZ) {
      ...
      const is0 = p.is0();
      if (iz == null)
        iz = is0 ? Fp.create(_8n3) : Fp.inv(Z);
      const x = Fp.mul(X, iz);
      const y = Fp.mul(Y, iz);
      const zz = Fp.mul(Z, iz);
      if (is0)
...
function cmask(P, swap) {
  return P + swap - (swap >> _1n10 << _1n10);
}
function cswap(P) {
...
function montgomery(curveDef) {
  ...
  function decodeScalar(scalar) {
    return bytesToNumberLE2(adjustScalarBytes(copyBytes(abytes3(scalar, fieldLen, "scalar"))));
  }
...
function abytes4(value, length, title = "") {
  if (isBytes5(value) && (length === undefined || value.length === length))
    return value;
  if (length !== undefined)
...
var wrapCipher = (params, constructor) => {
  function wrappedCipher(key, ...args) {
    ...
    if (params.withAAD && args[aadStart] !== undefined)
      abytes4(args[aadStart], undefined, "AAD");
...
var sharedKey = (sharedSecret) => sha2563(sharedSecret);
var decryptForCre = (envelope, recipientPrivateKey, base64ToBytes) => {
  const sharedSecret = x25519.getSharedSecret(recipientPrivateKey, base64ToBytes(envelope.ephemeralPublicKey));
  return new TextDecoder().decode(xchacha20poly1305(sharedKey(sharedSecret), base64ToBytes(envelope.nonce)).decrypt(base64ToBytes(envelope.payload)));
};
...
var isEncryptedBatch = (raw) => {
  if (!raw || typeof raw !== "object")
    return false;
  const obj = raw;
  return obj.encoding === "nugget1-contribution-batch-v1" && typeof obj.epoch === "string" && Array.isArray(obj.contributions);
};
...
var unlockContributionBatch = (body, privateKeyBase64) => {
  let parsed;
  try {
  ...
  const privateKey = base64ToBytes2(privateKeyBase64);
  if (privateKey.length !== 32)
    throw new Error("CRE private key must be 32 bytes");
